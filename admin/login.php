...
<title>Inloggen — <?= e(Settings::get('site_name', 'ROICT CMS')) ?> Beheer</title>
...
sl-input { --sl-input-border-color: #e2e8f0; --sl-input-border-color-focus: #2563eb; --sl-focus-ring-color: rgba(37,99,235,.4); --sl-input-border-radius-medium: 10px; }
sl-alert { --sl-border-radius-medium: 10px; }
.btn-login { background: linear-gradient(135deg, #2563eb, #7c3aed); border: none; color: white; width: 100%; padding: .75rem; border-radius: 10px; font-weight: 700; font-size: 1rem; cursor: pointer; transition: opacity .2s; min-height: 44px; }
.btn-login:hover { opacity: .9; }
.btn-login:focus-visible { outline: 3px solid #2563eb; outline-offset: 2px; opacity: .9; }
a:focus-visible { outline: 3px solid #2563eb; outline-offset: 2px; border-radius: 4px; }
</style>
</head>
<body>
<div class="container d-flex justify-content-center align-items-center" style="min-height:100vh;">
  <div class="login-card">
    ...
    <div class="login-body">
      ...
      <?php endif; ?>
      <form method="POST" autocomplete="on" aria-label="Inlogformulier">
        <?= csrf_field() ?>
        <div style="margin-bottom:1rem;">
          <sl-input label="Gebruikersnaam of e-mailadres" type="text" name="username"
            value="<?= e($_POST['username'] ?? '') ?>" autofocus required
            autocomplete="username">
            <i slot="prefix" class="bi bi-person" aria-hidden="true"></i>
          </sl-input>
        </div>
        <div style="margin-bottom:1.5rem;">
          <sl-input label="Wachtwoord" type="password" name="password" required password-toggle
            autocomplete="current-password">
            <i slot="prefix" class="bi bi-lock" aria-hidden="true"></i>
          </sl-input>
        </div>
      ...
      <div class="text-center mt-4">
        <a href="<?= BASE_URL ?>/" style="color:#64748b;font-size:.85rem;text-decoration:none;">
          <i class="bi bi-arrow-left" aria-hidden="true"></i> Terug naar website
        </a>
      </div>
